...
public class SimpleKafkaConsumer {
	...
	/**
	 * This function will start a single worker thread per topic. After creating the
	 * consumer object, we subscribed to a list of Kafka topics, in the constructor.
	 * For this example, the list consists of only one topic. But you can give it a
	 * try with multiple topics.
	 * @throws Exception 
	 */
	public void runSingleWorker() throws Exception {
		 ...
		 */
		BufferedWriter writer = new BufferedWriter(new FileWriter("c://kafka//outputkafka.dat", true));
		while (true) {
			...
			for (ConsumerRecord<String, String> record : records) {
				...
				/*
				 * If you remember, we sent 10 messages to this topic as plain strings. 10 other
				 * messages were serialized JSON objects. Now we'll deserialize them here. But
				 * since we can't make out which message is a serialized JSON object and which
				 * isn't, we'll try to deserialize all of them. So, obviously, we'll get an
				 * exception for the first 10 messages we receive. We'll just log the errors and
				 ...
				 */
				  ...
				  commitMessage.put(new TopicPartition(record.topic(), record.partition()), new
				  OffsetAndMetadata(record.offset() + 1));
				  
				  kafkaConsumer.commitSync(commitMessage);
				  
				  logger.info("Offset committed to Kafka." + commitMessage); }
